...
async def scan_market_signals(top_n: int = 10) -> List[SignalScanItem]:
    """
    Scans the equity universe for high-probability multi-factor trade opportunities.
    """
    ...
    for sym in symbols_to_scan:
        try:
            ...
            logger.warning(f"Signal scanner error for {sym}: {str(e)}")

    # Sort by strongest opportunities (highest composite score)
